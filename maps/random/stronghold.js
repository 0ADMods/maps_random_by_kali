...
switch(r) {
	...
	case 1:
		...
		group = new SimpleGroup(
			[new SimpleObject(oMetalLarge, 3, 6, 5, floor(mapSize * 0.15))],
		...
		group = new SimpleGroup(
			[new SimpleObject(oStoneLarge, 3, 6, 5, floor(mapSize * 0.15))],
			true, clBaseResource, centerOfMap, centerOfMap
		);
		createObjectGroup(group, 0, [avoidClasses(clBaseResource, 2, clPlayer, 5), stayClasses(clHill, 10)]);
		group = new SimpleGroup(
	...
	case 2:
		...
		if(makeMinerals) {
			...
			createMines(
			[
				[new SimpleObject(oStoneLarge, 2, 4, 50, mapSize)], [new SimpleObject(oStoneSmall, 2, 4, 50, mapSize)]
			],
			...
			createMines(
			[
				[new SimpleObject(oMetalLarge, 2, 4, 50, mapSize)]
			],
		...
		for(var a = 0; a < randAnimals; ++a) {
			...
			if(randType == 1) {
				group = new SimpleGroup(
					[new SimpleObject(oMainHuntableAnimal, 6, 10, 5, 15)],
	...
	case 3:
		...
		for(var w = 0; w < randPonds; ++w) {
			...
			group = new SimpleGroup(
				[new SimpleObject(oFish, 5, 9, 2, floor(mapSize * 0.15))],
				true, clBaseResource, randX, randY
			);
	...
	case 4:
		...
		group = new SimpleGroup(
			[new SimpleObject(oMainHuntableAnimal, 40, 50, floor(mapSize * 0.15), mapSize)],
			true, clBaseResource, centerOfMap, centerOfMap
		);
		createObjectGroup(group, 0, avoidClasses(clBaseResource, 1, clPlayer, 1, clWater, 1, clHill, 1));
		group = new SimpleGroup(
			[new SimpleObject(oFish, numPlayers, numPlayers * 3, 2, mapSize)],
			true, clBaseResource, centerOfMap, centerOfMap
		);
		createObjectGroup(group, 0, [avoidClasses(clLand, 1), stayClasses(clWater, 5)]);
		//makeMinerals = false;
	...
	case 7:
		...
		for(var w = 0; w < randPonds; ++w) {
			...
			group = new SimpleGroup(
				[new SimpleObject(oFish, 10, 15, 2, floor(mapSize * 0.25))],
				true, clBaseResource, randX, randY
			);
			createObjectGroup(group, 0, [avoidClasses(clLand, 1), stayClasses(clWater, 5)]);
		}
		...
		case 10:
			...
			// create center bounty
			group = new SimpleGroup(
				[new SimpleObject(oMetalLarge, 1, 4, 5, floor(mapSize * 0.15))],
				true, clBaseResource, centerOfMap, centerOfMap
			);
			createObjectGroup(group, 0, avoidClasses(clBaseResource, 5, clPlayer, 5));
			group = new SimpleGroup(
				[new SimpleObject(oStoneLarge, 1, 4, 5,floor(mapSize * 0.15))],
				true, clBaseResource, centerOfMap, centerOfMap
			);
			createObjectGroup(group, 0, avoidClasses(clBaseResource, 5, clPlayer, 5));
			group = new SimpleGroup(
		...
		case 11:
			...
			// create center bounty
			group = new SimpleGroup(
				[new SimpleObject(oMetalLarge, 2, 3, 5, floor(mapSize * 0.15))],
				true, clBaseResource, centerOfMap, centerOfMap
			);
			createObjectGroup(group, 0, [avoidClasses(clBaseResource, 5, clPlayer, 5),stayClasses(clLand, 10)]);
			group = new SimpleGroup(
				[new SimpleObject(oStoneLarge, 2, 3, 5, floor(mapSize * 0.15))],
				true, clBaseResource, centerOfMap, centerOfMap
			);
			createObjectGroup(group, 0, [avoidClasses(clBaseResource, 5, clPlayer, 5),stayClasses(clLand, 10)]);
			group = new SimpleGroup(
		...
		case 15:
			...
			// create center bounty
			group = new SimpleGroup(
				[new SimpleObject(oMetalLarge, 2, 4, 5, floor(mapSize * 0.1))],
				true, clBaseResource, centerOfMap, centerOfMap
			);
			createObjectGroup(group, 0, [avoidClasses(clBaseResource, 10, clPlayer, 25),stayClasses(clLand,10)]);
			group = new SimpleGroup(
				[new SimpleObject(oStoneLarge, 2, 4, 5, floor(mapSize * 0.1))],
				true, clBaseResource, centerOfMap, centerOfMap
			);
			createObjectGroup(group, 0, [avoidClasses(clBaseResource, 10, clPlayer, 25),stayClasses(clLand,10)]);
			break;
...
if(makeMinerals) {
	...
	createMines(
	[
		[new SimpleObject(oStoneLarge, 1, 3, 30, mapSize)], [new SimpleObject(oStoneSmall, 1, 1, 30, mapSize)]
	],
	avoidClasses(clWater, 2, clForest, 1, clPlayer, 20, clMetal, 10, clRock, 20, clHill, 2, clBaseResource, 10),
	clRock
	)
	...
	createMines(
	[
		[new SimpleObject(oMetalLarge, 1, 3, 30, mapSize)]
	],
	avoidClasses(clWater, 2, clForest, 1, clPlayer, 20, clMetal, 20, clRock, 10, clHill, 2, clBaseResource, 10),
	clMetal
	)
}
